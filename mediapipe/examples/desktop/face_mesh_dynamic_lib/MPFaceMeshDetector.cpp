...
absl::Status MPFaceMeshDetector::InitFaceMeshDetector(int numFaces,
  ...
  if (face_detection_model_path == nullptr) {
    face_detection_model_path = "mediapipe/modules/face_detection/face_detection_short_range.tflite";
  }

  if (face_landmark_model_path == nullptr) {
    face_landmark_model_path = "mediapipe/modules/face_landmark/face_landmark.tflite";
  }

  // Prepare graph config.
  auto preparedGraphConfig = absl::StrReplaceAll(graphConfig,
                                                 {{"$numFaces", std::to_string(numFaces)}});
...
MPFaceMeshDetector::DetectFacesWithStatus(const cv::Mat &camera_frame,
  ...
  if (!numFaces || !multi_face_bounding_boxes) {
    return absl::InvalidArgumentError("MPFaceMeshDetector::DetectFacesWithStatus requires notnull"
                                      "pointer to save results data.");
  }
  ...
  *numFaces = 0;
  ...
  const auto &face_bounding_boxes =
      face_rects_from_landmarks_packet
...
void MPFaceMeshDetector::DetectFaces(const cv::Mat &camera_frame,
                                     cv::Rect *multi_face_bounding_boxes,
                                     int *numFaces) {
  const auto status = DetectFacesWithStatus(camera_frame, multi_face_bounding_boxes, numFaces);
