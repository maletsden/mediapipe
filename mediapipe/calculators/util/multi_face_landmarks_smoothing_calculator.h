...
#include "mediapipe/util/filtering/relative_velocity_filter_with_fps_input.h"

namespace mediapipe {

namespace {

constexpr char kNormalizedLandmarksTag[] = "NORM_MULTI_FACE_LANDMARKS";
constexpr char kLandmarksTag[] = "MULTI_FACE_LANDMARKS";
constexpr char kImageSizeTag[] = "IMAGE_SIZE";
constexpr char kFPSTag[] = "FPS";
constexpr char kObjectScaleRoiTag[] = "OBJECT_SCALE_ROI";
constexpr char kNormalizedFilteredLandmarksTag[] = "NORM_FILTERED_MULTI_FACE_LANDMARKS";
constexpr char kFilteredLandmarksTag[] = "FILTERED_MULTI_FACE_LANDMARKS";

using mediapipe::OneEuroFilter;
using mediapipe::RelativeVelocityFilterWithFpsInput;
  ...
  absl::Status Apply(const LandmarkList& in_landmarks,
    ...
    for (int i = 0; i < in_landmarks.landmark_size(); ++i) {
      ...
      *out_landmark = in_landmark;
      out_landmark->set_x(
          x_filters_[i].Apply(timestamp, value_scale, fps, in_landmark.x()));
      out_landmark->set_y(
          y_filters_[i].Apply(timestamp, value_scale, fps, in_landmark.y()));
      out_landmark->set_z(
          z_filters_[i].Apply(timestamp, value_scale, fps, in_landmark.z()));
    }
 ...
 private:
  ...
  absl::Status InitializeFiltersIfEmpty(const int n_landmarks) {
    ...
    x_filters_.resize(n_landmarks,
                      RelativeVelocityFilterWithFpsInput(window_size_, velocity_scale_));
  ...
  std::vector<RelativeVelocityFilterWithFpsInput> x_filters_;
  std::vector<RelativeVelocityFilterWithFpsInput> y_filters_;
  std::vector<RelativeVelocityFilterWithFpsInput> z_filters_;
};
...
//     input_stream: "NORM_MULTI_FACE_LANDMARKS:pose_landmarks"
//     input_stream: "IMAGE_SIZE:image_size"
//     input_stream: "OBJECT_SCALE_ROI:roi"
//     output_stream: "NORM_FILTERED_MULTI_FACE_LANDMARKS:pose_landmarks_filtered"
//     options: {
...
//   }
//
class MultiFaceLandmarksSmoothingCalculator : public CalculatorBase {
 public:
...
REGISTER_CALCULATOR(MultiFaceLandmarksSmoothingCalculator);

absl::Status MultiFaceLandmarksSmoothingCalculator::GetContract(CalculatorContract* cc) {
    if (cc->Inputs().HasTag(kNormalizedLandmarksTag)) {
    cc->Inputs().Tag(kNormalizedLandmarksTag).Set<std::vector<NormalizedLandmarkList>>();
    cc->Inputs().Tag(kImageSizeTag).Set<std::pair<int, int>>();
    cc->Inputs().Tag(kFPSTag).Set<int>();
    cc->Outputs()
    ...
    cc->Inputs().Tag(kFPSTag).Set<int>();
    cc->Outputs().Tag(kFilteredLandmarksTag).Set<std::vector<LandmarkList>>();
...
absl::Status MultiFaceLandmarksSmoothingCalculator::Open(CalculatorContext* cc) {
...
absl::Status MultiFaceLandmarksSmoothingCalculator::Process(CalculatorContext* cc) {
  ...
  if (cc->Inputs().HasTag(kNormalizedLandmarksTag)) {
    const auto& in_norm_multi_face_landmarks =
        cc->Inputs().Tag(kNormalizedLandmarksTag).Get<std::vector<NormalizedLandmarkList>>();
    std::vector<NormalizedLandmarkList> out_norm_multi_face_landmarks;
    int image_width;
    int image_height;
    std::tie(image_width, image_height) =
        cc->Inputs().Tag(kImageSizeTag).Get<std::pair<int, int>>();

    int fps = cc->Inputs().Tag(kFPSTag).Get<int>();
    ...
    for (auto& in_norm_landmarks : in_norm_multi_face_landmarks) {
        ...
        MP_RETURN_IF_ERROR(landmarks_filter_->Apply(
            *in_landmarks, timestamp, object_scale, fps, out_landmarks.get()));
        ...
        out_norm_multi_face_landmarks.push_back(*out_norm_landmarks);
    }
    cc->Outputs()
        .Tag(kNormalizedFilteredLandmarksTag)
        .AddPacket(mediapipe::MakePacket<std::vector<NormalizedLandmarkList>>(out_norm_multi_face_landmarks)
            .At(cc->InputTimestamp()));
  } else {
    const auto& in_multi_face_landmarks =
        cc->Inputs().Tag(kLandmarksTag).Get<std::vector<LandmarkList>>();
    int fps = cc->Inputs().Tag(kFPSTag).Get<int>();
    std::vector<LandmarkList> out_multi_face_landmarks;
    absl::optional<float> object_scale;
    if (cc->Inputs().HasTag(kObjectScaleRoiTag) &&
        !cc->Inputs().Tag(kObjectScaleRoiTag).IsEmpty()) {
    ...
    for (auto& in_landmarks : in_multi_face_landmarks) {
        auto out_landmarks = absl::make_unique<LandmarkList>();
        MP_RETURN_IF_ERROR(landmarks_filter_->Apply(
            in_landmarks, timestamp, object_scale, fps, out_landmarks.get()));
        out_multi_face_landmarks.push_back(*out_landmarks.release());
    }

    cc->Outputs()
        .Tag(kFilteredLandmarksTag)
        .Add(&out_multi_face_landmarks, cc->InputTimestamp());
  }
